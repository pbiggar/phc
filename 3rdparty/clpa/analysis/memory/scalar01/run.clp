...
predicate failure(P:pp,I:c_instr,ASN:g_guard_asn).
...
icall(P,_,I), dircall(I,"assert"), cil_instr_call_arg(I,0,E),
    ...
    #and(TG0,GG,TG), guard_sat_asn(TG,ASN), +failure(P,I,ASN).

failure(_,I,_), cil_instr_x_location(I,_,LINE), +failed_assert(LINE).
failure(P,I,ASN), ?val(P, root{S}, _, _), val(P, root{S}, T, _), 
    asn_value(ASN, T, N), cil_instr_x_location(I,_,LINE), 
    +cexample(LINE,S,N).
...
cexample(206,arg{0},NA), cexample(206,arg{1},NB), int_ge(NA,NB), 
    int_add(NB,10,NP), int_le(NA,NP), +correct(206).

cexample(304,arg{0},NA), cexample(304,local{"x"},NX), 
    cexample(304,local{"y"},NY), int_ne(NA,NX), int_eq(NA,NY), +correct(304).
