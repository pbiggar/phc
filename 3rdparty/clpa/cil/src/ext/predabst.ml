...
module Solver = functor(T:TRANSLATOR) ->
  struct
    ...
    let rec transExp e =
      match e with
      | Const(CInt64(v,k,so), _) -> T.mkConst (Int64.to_int v)
      | Const _ -> raise NYI
      | Lval((Var vi,NoOffset),_) when vi.vname = "_ZERO_" ->
	  T.mkConst 0
      | Lval (l,_) -> T.mkVar (sprint 80 (d_lval () l))
      | UnOp(op,e,_,_) -> 
	  let e = transExp e in
	  transUnOp op e
      | BinOp(op,e1,e2,_,_) ->
	  ...
	  transBinOp op e1 e2
      | SizeOf (typ,_) -> T.mkConst ((bitsSizeOf typ)/8)
      | SizeOfE (e,loc) -> transExp (SizeOf(typeOf e, loc))
      | SizeOfStr (s,_) -> T.mkConst (1 + String.length s)
      | AlignOf (typ,_) -> T.mkConst (alignOf_int typ)
      | AlignOfE (e,loc) -> transExp (AlignOf(typeOf e, loc))
      | CastE(typ,e,_) -> transExp  e
      (* Cast should check if signed type, and if so, make an ite term *)
      | AddrOf (lv,_) -> T.mkVar (sprint 80 (d_exp () e))
      | StartOf (lv,_) -> T.mkVar (sprint 80 (d_exp () e))
...
module PredAbst = functor(S:SOLVER) ->
  struct
    ...
    class returnFinderClass vor = object(self)
      ...
      | Return(Some(Lval((Var vi,NoOffset),_)),_) -> begin
	  vor := Some(vi);
	  SkipChildren
      end
      | _ -> DoChildren
	...
	=
      ...
      let rec helper e =
	match e with
	| Const _ -> [(one unknownExpLoc,e)]
	| AddrOf((Var v, NoOffset),_) -> [(one unknownExpLoc, e)]
	| StartOf((Var v, NoOffset),_) -> [(one unknownExpLoc, e)]
	| Lval((Mem me, NoOffset),loc) ->
	    if Ptranal.may_alias me (AddrOf (lv,loc)) then
	      [(BinOp(Eq,me,AddrOf (lv,loc),intType,loc),
		rhse);
	       (UnOp(LNot,BinOp(Eq,me,AddrOf (lv,loc),intType,loc),intType,loc),
		e)]
	    else [(one unknownExpLoc, e)]
	| Lval((Var v, off),_) ->
	    if compareLval (Var v, off) lv then
	      [(one unknownExpLoc,rhse)]
	    else
	      [(one unknownExpLoc,e)]
	| BinOp(op, e1, e2,t,loc) ->
		...
		(BinOp(LAnd,c1,c2,intType,loc),BinOp(op,e1,e2,t,loc))) pl2))
	      [] pl1
	| UnOp(op, e, t,loc) ->
	    let pl = helper e in
	    List.map (fun (c,e) ->
	...
	List.fold_left (fun d (c,e) ->
	  BinOp(LOr,d,BinOp(LAnd,c,e,intType,unknownExpLoc),intType,unknownExpLoc))
	  (zero unknownExpLoc) pl
      in
      let rec cleanUpExp e = match e with
      | BinOp(LAnd,e1,e2,_,_) when compareExp (cleanUpExp e1) (one unknownExpLoc) -> 
	  cleanUpExp e2
      | BinOp(LAnd,e1,e2,_,_) when compareExp (cleanUpExp e2) (one unknownExpLoc) -> 
	  cleanUpExp e1
      | BinOp(LOr,e1,e2,_,_) when compareExp (cleanUpExp e1) (zero unknownExpLoc)-> 
	  cleanUpExp e2
      | BinOp(LOr,e1,e2,_,_) when compareExp (cleanUpExp e2) (zero unknownExpLoc) -> 
	  cleanUpExp e1
      | UnOp(LNot,e,_,_) when compareExp (cleanUpExp e) (one unknownExpLoc) -> 
	  (zero unknownExpLoc)
      | UnOp(LNot,e,_,_) when compareExp (cleanUpExp e) (zero unknownExpLoc) -> 
	  (one unknownExpLoc)
      | _ -> e
      in
	...
	IH.fold (fun pid bl pre ->
	  ...
	  match bl with
	  | Top | Bottom -> pre
	  | True -> BinOp(LAnd,pre,getPred ctxt pid,intType,unknownExpLoc)
	  | False -> BinOp(LAnd,pre,UnOp(LNot,getPred ctxt pid,intType,unknownExpLoc),intType,unknownExpLoc))
	  inss (one unknownExpLoc)
      in
      let pre = 
	List.fold_left (fun ce e ->
	  BinOp(LAnd,ce,e,intType,unknownExpLoc))
	  pre extra
      in
      IH.iter (fun pid bl ->
	...
	else if S.isValid (S.transExp pre) (S.transExp (UnOp(LNot,e,intType,unknownExpLoc))) 
	then
	...
	=
      match i with
      | Call(lvo, Lval((Var vi, NoOffset),_), el, _) -> begin
	  ...
	  let rpreds =
		...
		List.map (fun e ->
		  substitute (Lval (lv,unknownExpLoc)) (* for *) (Var rvi,NoOffset) (* in *) e)
		  rpreds
	...
	=
      ...
      let args =
	match reto with
	| Some lv -> (Lval (lv,unknownExpLoc))::args
	| None -> args
      in
      IH.iter (fun pid bl ->
	let e = getPred ctxt pid in
	  ...
	  let newhl = 
	    List.fold_left (fun inss (i,oldss) ->
		 ...
		 =>give up! (but fix later)
	      ...
	      | Call(lvo,Lval((Var vi,NoOffset),_),el,l)
