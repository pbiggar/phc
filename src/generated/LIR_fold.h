...
public:
   typedef Fold<_API_CALL, _Action, _Allocate, _Assign_zvp, _Assign_zvpp, _Block, _CODE, _C_file, _Clone, _Cond, _Dec_ref, _Deref, _Destruct, _INTRINSIC, _If, _Inc_ref, _Is_ref, _LITERAL, _Node, _Null, _Piece, _Ref, _Separate, _Statement, _UNINTERPRETED, _Uninitialized, _ZVP, _ZVPP, _Zvp, _Zvpp, _List> parent;
...
public:
	...
	virtual _Block fold_block(Block* in)
	{
		...
		return fold_impl_block(in, statements);
	}
	...
	virtual _If fold_if(If* in)
	{
		...
		_List<_Statement>* if_true = 0;
	
		{
			if_true = new _List<_Statement>;
			typename _List<Statement*>::const_iterator i;
			for(i = in->if_true->begin(); i != in->if_true->end(); i++)
				if(*i != NULL) if_true->push_back(fold_statement(*i));
				else if_true->push_back(0);
		}
		_List<_Statement>* if_false = 0;
	
		{
			if_false = new _List<_Statement>;
			typename _List<Statement*>::const_iterator i;
			for(i = in->if_false->begin(); i != in->if_false->end(); i++)
				if(*i != NULL) if_false->push_back(fold_statement(*i));
				else if_false->push_back(0);
		}
...
public:
	...
	virtual _Block fold_impl_block(Block* orig, _List<_Statement>* statements) { assert(0); };
...
class Uniform_fold : public Fold<T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, _List> {};
}
