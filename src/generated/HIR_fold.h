...
public:
   typedef Fold<_Actual_parameter, _Assign_array, _Assign_target, _Assign_var, _Assign_var_var, _Attr_mod, _Attribute, _BOOL, _Bin_op, _Break, _CAST, _CLASS_NAME, _CONSTANT_NAME, _Cast, _Catch, _Class_def, _Class_mod, _Class_name, _Constant, _Continue, _Eval_expr, _Expr, _Foreach, _Foreign, _Formal_parameter, _Global, _INT, _INTERFACE_NAME, _Identifier, _If, _Index_array, _Instanceof, _Interface_def, _Literal, _Loop, _METHOD_NAME, _Member, _Method, _Method_invocation, _Method_mod, _Method_name, _NIL, _Name_with_default, _New, _Node, _OP, _PHP_script, _Pre_op, _Push_array, _REAL, _Return, _Rvalue, _STRING, _Signature, _Statement, _Static_array, _Static_array_elem, _Static_array_key, _Static_declaration, _Static_value, _Target, _Target_expr, _Throw, _Try, _Type, _Unary_op, _VARIABLE_NAME, _Variable_class, _Variable_method, _Variable_name, _Variable_variable, _List> parent;
...
public:
	...
	{
		_VARIABLE_NAME arr = 0;
		if(in->arr != NULL) arr = fold_variable_name(in->arr);
		_VARIABLE_NAME key = 0;
		if(in->key != NULL) key = fold_variable_name(in->key);
		bool is_ref = in->is_ref;
		_VARIABLE_NAME val = 0;
		if(in->val != NULL) val = fold_variable_name(in->val);
		_List<_Statement>* statements = 0;
		...
		return fold_impl_foreach(in, arr, key, is_ref, val, statements);
	}
...
public:
	...
	virtual _If fold_impl_if(If* orig, _VARIABLE_NAME variable_name, _List<_Statement>* iftrue, _List<_Statement>* iffalse) { assert(0); };
	virtual _Loop fold_impl_loop(Loop* orig, _List<_Statement>* statements) { assert(0); };
	...
	virtual _Assign_target fold_impl_assign_target(Assign_target* orig, _Target target, _Variable_name lhs, bool is_ref, _Rvalue rhs) { assert(0); };
	...
	virtual _Index_array fold_impl_index_array(Index_array* orig, _VARIABLE_NAME variable_name, _Rvalue index) { assert(0); };
	...
	virtual _Actual_parameter fold_impl_actual_parameter(Actual_parameter* orig, bool is_ref, _Target target, _Variable_name variable_name, _List<_Rvalue>* array_indices) { assert(0); };
...
class Uniform_fold : public Fold<T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, _List> {};
}
