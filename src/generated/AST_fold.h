...
public:
	...
	{
		List<_Directive>* directives = 0;
	
		{
			directives = new List<_Directive>;
	...
	{
		_Variable array = 0;
		if(in->array != NULL) array = fold_variable(in->array);
		_HT_ITERATOR iter = 0;
...
public:
	...
	virtual _Foreach_reset fold_impl_foreach_reset(Foreach_reset* orig, _Variable array, _HT_ITERATOR iter) { assert(0); };
	virtual _Foreach_next fold_impl_foreach_next(Foreach_next* orig, _Variable array, _HT_ITERATOR iter) { assert(0); };
	virtual _Foreach_end fold_impl_foreach_end(Foreach_end* orig, _Variable array, _HT_ITERATOR iter) { assert(0); };
	virtual _Foreach_has_key fold_impl_foreach_has_key(Foreach_has_key* orig, _Variable array, _HT_ITERATOR iter) { assert(0); };
	virtual _Foreach_get_key fold_impl_foreach_get_key(Foreach_get_key* orig, _Variable array, _HT_ITERATOR iter) { assert(0); };
	virtual _Foreach_get_val fold_impl_foreach_get_val(Foreach_get_val* orig, _Variable array, _Variable key, _HT_ITERATOR iter) { assert(0); };
