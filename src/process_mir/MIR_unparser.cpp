...
MIR_unparser::MIR_unparser (ostream& os) : PHP_unparser (os)
{
}

void MIR_unparser::children_php_script(PHP_script* in)
{
	echo_nl("<?php");
...
void MIR_unparser::children_interface_def(Interface_def* in)
{
	echo("interface ");
...
void MIR_unparser::children_class_def(Class_def* in)
{
...
void MIR_unparser::children_class_mod(Class_mod* in)
{
	if(in->is_abstract) echo("abstract ");
	if(in->is_final) echo("final ");
}

void MIR_unparser::children_method(Method* in)
{
...
void MIR_unparser::children_signature(Signature* in)
{
...
void MIR_unparser::children_method_mod(Method_mod* in)
{
...
void MIR_unparser::children_formal_parameter(Formal_parameter* in)
{
	visit_type(in->type);
	if(in->is_ref) echo("&");
...
void MIR_unparser::children_type(Type* in)
{
...
void MIR_unparser::children_attribute(Attribute* in)
{
...
void MIR_unparser::children_attr_mod(Attr_mod* in)
{
	if(in->is_public)		echo("public ");
	if(in->is_protected)	echo("protected ");
	if(in->is_private)	echo("private ");
...
void MIR_unparser::children_return(Return* in)
{
	echo("return");
	if(in->expr != NULL)
	{
...
void MIR_unparser::children_static_declaration(Static_declaration* in)
{
	echo("static ");
	visit_name_with_default(in->var);
	echo(";");
...
void MIR_unparser::children_global(Global* in)
{
	echo("global $");
...
void MIR_unparser::children_try(Try* in)
{
	echo("try");
	space_or_newline();
...
void MIR_unparser::children_catch(Catch* in)
{
	echo("catch (");
	visit_class_name(in->class_name);
	echo(" $");
...
void MIR_unparser::post_catch_chain(Catch* in)
{
...
void MIR_unparser::children_throw(Throw* in)
{
	echo("throw ");
	visit_expr(in->expr);
	echo(";");
...
void MIR_unparser::children_eval_expr(Eval_expr* in)
{
	visit_expr(in->expr);
	echo(";");
...
void MIR_unparser::children_assignment(Assignment* in)
{
...
void MIR_unparser::children_cast(Cast* in)
{
	echo("(");
	visit_cast(in->cast);
	echo(") ");
...
void MIR_unparser::children_unary_op(Unary_op* in)
{
	visit_op(in->op);
	visit_variable_name(in->variable_name);
}

void MIR_unparser::children_bin_op(Bin_op* in)
{
...
void MIR_unparser::children_constant(Constant* in)
{
	if(in->class_name != NULL)
	{
		visit_class_name(in->class_name);
...
void MIR_unparser::children_instanceof(Instanceof* in)
{
	visit_variable_name(in->variable_name);
	echo(" instanceof ");
	visit_class_name(in->class_name);
...
void MIR_unparser::children_variable(Variable* in)
{
...
void MIR_unparser::children_reflection(Reflection* in)
{
	visit_variable_name (in->variable_name);
}

void MIR_unparser::children_pre_op(Pre_op* in)
{
	visit_op(in->op);
	visit_variable(in->variable);
}

void MIR_unparser::children_array(Array* in)
{
	echo("array");
	visit_array_elem_list(in->array_elems);
}

void MIR_unparser::children_array_elem(Array_elem* in)
{
	if(in->key != NULL)
	{
		visit_expr(in->key);
...
void MIR_unparser::children_method_invocation(Method_invocation* in)
{
...
void MIR_unparser::children_actual_parameter(Actual_parameter* in)
{
	if(in->is_ref) echo("&");
	visit_target (in->target);
	visit_variable_name (in->variable_name);
...
void MIR_unparser::children_new(New* in)
{
	echo("new ");
	visit_class_name(in->class_name);
	echo("(");
...
void MIR_unparser::visit_interface_name_list(List<INTERFACE_NAME*>* in)
{
...
void MIR_unparser::visit_member_list(List<Member*>* in)
{
	newline();
	echo_nl("{");
	inc_indent();
...
void MIR_unparser::visit_statement_list(List<Statement*>* in)
{
	echo("{");
	inc_indent();
...
void MIR_unparser::visit_formal_parameter_list(List<Formal_parameter*>* in)
{
	echo("(");
...
void MIR_unparser::visit_catch_list(List<Catch*>* in)
{
...
void MIR_unparser::visit_expr_list(List<Expr*>* in)
{
...
void MIR_unparser::visit_array_elem_list(List<Array_elem*>* in)
{
	echo("(");
...
void MIR_unparser::visit_actual_parameter_list(List<Actual_parameter*>* in)
{
...
void MIR_unparser::visit_name_with_default_list(List<Name_with_default*>* in)
{
...
// Token classes
void MIR_unparser::children_interface_name(INTERFACE_NAME* in)
{
	echo(in->value);
}

void MIR_unparser::children_class_name(CLASS_NAME* in)
{
	echo(in->value);
}

void MIR_unparser::children_method_name(METHOD_NAME* in)
{
	echo(in->value);
}
...
void MIR_unparser::children_op(OP* in)
{
	echo(in->value);
}

void MIR_unparser::children_constant_name(CONSTANT_NAME* in)
{
	echo(in->value);
}

void MIR_unparser::children_int(INT* in)
{
	echo(in->get_value_as_string());
}

void MIR_unparser::children_real(REAL* in)
{
	echo(in->source_rep);
}
...
void MIR_unparser::children_bool(BOOL* in)
{
	echo(in->get_value_as_string());
}

void MIR_unparser::children_nil(NIL* in)
{
	echo(in->get_value_as_string());
}
...
void MIR_unparser::pre_variable (Variable* in)
{
...
void MIR_unparser::pre_method_invocation (Method_invocation* in)
{
...
void MIR_unparser::pre_global (Global* in)
{
	Reflection* reflect = dynamic_cast<Reflection*>(in->variable_name);
...
void MIR_unparser::pre_expr(Expr* in)
{
...
void MIR_unparser::post_expr(Expr* in)
{
	if(in->attrs->is_true("phc.unparser.needs_curlies") or in->attrs->is_true("phc.unparser.needs_user_curlies"))
		echo("}");
	if(in->attrs->is_true("phc.unparser.needs_user_brackets"))
...
void 
MIR_unparser::children_goto (Goto* in)
{
	echo ("goto ");
...
void 
MIR_unparser::children_label (Label* in)
{
	visit_label_name (in->label_name);
	echo_nl(":");
}

void MIR_unparser::children_name_with_default (Name_with_default* in)
{
	echo("$");
	visit_variable_name(in->variable_name);
	if(in->expr != NULL)
