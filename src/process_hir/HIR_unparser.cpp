...
void HIR_unparser::children_if(If* in)
{
...
void HIR_unparser::children_while(While* in)
{
	echo("while (");
	visit_expr(in->expr);
	echo(")");
...
void HIR_unparser::children_do(Do* in)
{
	echo("do");
	space_or_newline();
	visit_statement_list(in->statements);
...
void HIR_unparser::children_for(For* in)
{
	echo("for(");
...
void HIR_unparser::children_foreach(Foreach* in)
{
	echo("foreach (");
	visit_expr(in->expr);
	echo(" as ");
...
{
	// If there is no key in the HIR, we need to unparse one anyway, so we can
	// access it.
	Expr* expr;
	...
	{
		// The key isnt in the HIR, so it was added as an attribute instead.
...
void HIR_unparser::children_switch(Switch* in)
{
	echo("switch (");
	visit_expr(in->expr);
	echo(")");
...
void HIR_unparser::children_switch_case(Switch_case* in)
{
	if(in->expr != NULL)
	{
		echo("case ");
...
void HIR_unparser::children_break(Break* in)
{
	echo("break");
	if(in->expr != NULL)
	{
...
void HIR_unparser::children_continue(Continue* in)
{
	echo("continue");
	if(in->expr != NULL)
	{
...
void HIR_unparser::children_declare(Declare* in)
{
	echo("declare");
...
void HIR_unparser::children_directive(Directive* in)
{
...
// $a += $b;
void HIR_unparser::children_op_assignment(Op_assignment* in)
{
		visit_variable(in->variable);
...
void HIR_unparser::children_list_assignment(List_assignment* in)
{
...
void HIR_unparser::children_conditional_expr(Conditional_expr* in)
{
	visit_expr(in->cond);
	echo(" ? ");
...
void HIR_unparser::children_ignore_errors(Ignore_errors* in)
{
	echo("@");
	visit_expr(in->expr);
}
...
void HIR_unparser::children_post_op(Post_op* in)
{
	visit_variable(in->variable);
	visit_op(in->op);
}
...
void HIR_unparser::visit_switch_case_list(List<Switch_case*>* in)
{
	echo_nl("{");
	inc_indent();
...
void HIR_unparser::visit_directive_list(List<Directive*>* in)
{
	echo("(");
...
void HIR_unparser::visit_list_element_list(List<List_element*>* in)
{
	echo("list(");
...
void HIR_unparser::children_directive_name(DIRECTIVE_NAME* in)
{
	echo(in->value);
}
...
void HIR_unparser::pre_commented_node(Commented_node* in)
{
	List<String*>::const_iterator i;
	List<String*>* comments = in->get_comments();
	for(i = comments->begin(); i != comments->end(); i++)
...
void HIR_unparser::post_commented_node(Commented_node* in)
{
	List<String*>::const_iterator i;
	List<String*>* comments = in->get_comments();
...
void HIR_unparser::children_goto (Goto* in)
{
	echo ("goto ");
...
void HIR_unparser::children_label (Label* in)
{
	visit_label_name (in->label_name);
	echo_nl(":");
}

void HIR_unparser::children_nop (Nop* in)
{
