  ...
  if (success == SUCCESS)
    {
      // run it
      success =
	zend_execute_scripts (ZEND_INCLUDE TSRMLS_CC, p_result, 1, &handle);
  ...
  if (success == SUCCESS)
    {
      // run it
      success =
	zend_execute_scripts (ZEND_REQUIRE TSRMLS_CC, p_result, 1, &handle);
      assert (success == SUCCESS);
      zend_stream_close (&handle);
    }
  else
    {
      php_error_docref ("function.require" TSRMLS_CC, E_ERROR,
